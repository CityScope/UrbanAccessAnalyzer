...
GRADE_COLOR_MAP = {
    'A': ['#68b684', '#40916c', '#1b4332'],  # Medium green → deeper green
    'B': ['#f1c40f', '#d4ac0d', '#b7950b'],  # Gold → amber shades
    'C': ['#ffa75a', "#fa8246", "#cf5600"],  # Peach → medium orange → burnt orange
    'D': ["#b30202"],                        # Red
    'E': ['#9b59b6'],                        # Purple
    'F': ["#0051ff"],                        # Blue-gray / deep slate
    None: ['transparent']
}
GENERAL_COLORS = {
    "A": "#229251",   # green
...
defaults = {
    ...
    "population_file": None, "adress_map" : None, "ls_map" : None, "raster_map" : None, 
    "pop_offer_df" : None, "pct_insufficient" : None,
}
...
# Helper functions for persisting folium maps
def save_map_to_html(map_obj):
    """Converts a folium Map object to its HTML string representation."""
    if map_obj is None:
        return None
    # Folium's internal method to get the full HTML representation
    return map_obj._repr_html_()

def display_saved_map(map_html, width=700, height=300):
    """Displays the saved HTML map using Streamlit components."""
    if map_html:
        # Use components.html to render the raw HTML map
        components.html(map_html, height=height, width=width)
...
def plot_service_intensity(service_intensity):
    ...
    if "file_id" in pdf.columns and hasattr(st.session_state, "gtfs_providers"):
        ...
        provider_map = {fid: str(provider_list[fid])[:30] if fid < len(provider_list) else f"File {fid}"
...
with st.sidebar:
    ...
    if addr_input != st.session_state.last_input:
        ...
        # RESET MAPS
        st.session_state.adress_map = None
        st.session_state.ls_map = None
        st.session_state.raster_map = None
            ...
            if st.button(s["display_name"],key=f"s{i}"):
                ...
                if res:
                    ...
                    try:
                        ...
                        st.warning(f"AOI could not be loaded. Using a 3km buffer: {e}")
                        aoi = gpd.GeoDataFrame(geometry=gpd.points_from_xy([lon],[lat],crs=4326))
                        aoi = aoi.to_crs(aoi.estimate_utm_crs()).buffer(5000)
                        st.session_state.aoi = aoi
                        st.session_state.aoi_download = aoi.to_crs(aoi.estimate_utm_crs()).buffer(BUFFER_LOS)
                        st.session_state.aoi_pop_download = aoi.to_crs(aoi.estimate_utm_crs()).buffer(BUFFER_POP)
...
def get_adress_map(addr,aoi=None):
    # Map of AOI
    ...
    if aoi is not None:
        folium.GeoJson(aoi.to_crs("EPSG:4326").geometry,
...
if st.session_state.confirmed_address:
    ...
    # Map of AOI
    if st.session_state.adress_map is None:
        adress_map = get_adress_map(addr, st.session_state.aoi)
        st.session_state.adress_map = save_map_to_html(adress_map)
    
    display_saved_map(st.session_state.adress_map, width=700, height=300)

    st.markdown("---")
    st.header("Search Configuration")

    # --- Search level selection ---
    previous_search_level = st.session_state.get("previous_search_level", None)
    ...
    # Update previous search level
    st.session_state.previous_search_level = search_level

    # --- Official feed toggle ---
    is_official_checkbox = st.checkbox("Check official feeds only", value=False, key="official_feeds_checkbox")
    is_official = True if is_official_checkbox else None

    no_date_bounds_checkbox = st.checkbox("Any available date", value=False, key="any_date_checkbox")
    no_date_bounds = True if no_date_bounds_checkbox else False

    # Only show search form and parameters if NOT "Area"
    if search_level != "Area":
        # --- Add search area ---
        with st.form("add_search_form", clear_on_submit=True):
            search_text = st.text_input("Add search address", placeholder="e.g., San Francisco, California")
            submitted = st.form_submit_button("Add search area")

            if submitted and search_text.strip():
                try:
                    new_geo = get_geographic_suggestions_from_string(search_text)
                    st.session_state.search_area["country_codes"] += new_geo["country_codes"]
                    st.session_state.search_area["country_codes"] = list(np.unique(st.session_state.search_area["country_codes"]))
                    st.session_state.search_area["subdivision_names"] += new_geo["subdivision_names"]
                    st.session_state.search_area["subdivision_names"] = list(np.unique(st.session_state.search_area["subdivision_names"]))
                    st.session_state.search_area["municipalities"] += new_geo["municipalities"]
                    st.session_state.search_area["municipalities"] = list(np.unique(st.session_state.search_area["municipalities"]))

                    st.success(
                        f"Added search area: {new_geo['municipalities']} "
                        f"({new_geo['subdivision_names']}, {new_geo['country_codes']})"
                    )
                except Exception as e:
                    st.error(f"Failed to add search area: {e}")
            ...
            try:
                if st.session_state.aoi_download is None:
                    ...
                    with st.spinner("Searching and downloading GTFS feeds..."):
                        ...
                        if search_level == 'City':
                            feeds = api.search_gtfs_feeds(
                                country_code=st.session_state.search_area["country_codes"],
                                subdivision_name=st.session_state.search_area["subdivision_names"],
                                municipality=st.session_state.search_area["municipalities"],
                                is_official=is_official
                            )
                        elif search_level == 'State':
                            feeds = api.search_gtfs_feeds(
                                country_code=st.session_state.search_area["country_codes"],
                                subdivision_name=st.session_state.search_area["subdivision_names"],
                                is_official=is_official
                            )
                        elif search_level == 'Country':
                            feeds = api.search_gtfs_feeds(
                                country_code=st.session_state.search_area["country_codes"],
                                is_official=is_official
                            )
                        elif search_level == 'Area':
                            feeds = api.search_gtfs_feeds(
                                aoi = st.session_state.aoi_download,
                                is_official=is_official
                            )   
                        ...
                        st.session_state.gtfs_providers = [f["provider"] for f in feeds]

                        files = api.download_feeds(
                            feeds,
                            download_folder=os.path.join(WORK_DIR, "gtfs_files"),
                            overwrite=False
                        )

                        start_dt = datetime.combine(date.today() + timedelta(days=1), time())
                        end_dt = datetime.combine(date.today() + timedelta(days=31), time())
...
def get_ls_map(gdf,aoi,level_of_service_gdf):
    ...
    service_quality_gdf = gdf_cropped.copy()
    ...
    los_gdf = level_of_service_gdf.to_crs(4326)
...
if st.session_state.gtfs_ready:
    ...
    s_int_df = s_int.to_pandas()
    weekday_services = s_int_df.loc[
        (s_int_df['holiday'] == False) & (s_int_df['weekend'] == False)
    ]
    ...
    if no_date_bounds:
            chosen_date = st.date_input("Date for Analysis", value=selected_weekday)
    else:
        chosen_date = st.date_input("Date for Analysis", value=selected_weekday, min_value=date.today() + timedelta(days=1))
    ...
    if show_step2_results:
        ...
        with col_dist:
            ...
            svc_dist = svc_dist.rename(columns={i : f"{i} m" for i in svc_dist.columns[1:]})
        ...
        if st.session_state.level_of_service_gdf is not None:
            ...
            if st.session_state.ls_map is None:
                stops_gdf = st.session_state.service_quality_gdf.copy()
                level_of_service_gdf = st.session_state.level_of_service_gdf.copy()
                aoi = st.session_state.aoi.copy()
                ls_map, stop_quality_gdf_cropped = get_ls_map(stops_gdf,aoi,level_of_service_gdf)
                
                # Save as HTML string
                st.session_state.ls_map = save_map_to_html(ls_map)
                st.session_state.stop_quality_gdf_cropped = stop_quality_gdf_cropped
            
            # Display saved HTML map
            display_saved_map(st.session_state.ls_map, width=1200, height=600)

        
# === STEP 3: Accessibility Analysis ===
def get_raster_map(aoi_geom,addr,stop_quality_gdf_cropped):
    ...
    for filename, config in files_to_plot.items():
        ...
        try:
            # We must project the AOI to the raster's CRS before reading
            with rasterio.open(filepath) as src:
                raster_crs = src.crs
            
            aoi_projected = aoi_geom.to_crs(raster_crs)

            data, transform, crs = raster_utils.read_raster(filepath, aoi=aoi_projected, projected=True)
            if data.size == 0:
                continue
            ...
            if filename == "population_density_0.tif":
                ...
                # Ensure data is plotted correctly
                if data_to_plot.count() > 0:
                    ax.imshow(data_to_plot, cmap=cmap, origin='upper', extent=(left, right, bottom, top))
                else:
                     ax.axis('off')

            elif filename == "difference.tif":
                # Ensure we load population data if it wasn't the first raster processed
                if pop_data_cache is None:
                    pop_filepath = os.path.join(level_of_service_path, "population_density_0.tif")
                    pop_data_raw, _, _ = raster_utils.read_raster(pop_filepath, aoi=aoi_projected, projected=True)
                    pop_data_cache = np.ma.masked_array(pop_data_raw.astype(np.float64), mask=~np.isfinite(pop_data_raw))
                
                # Raster values are densities, but here we use the boolean mask logic
                pop_mask = (pop_data_cache.data >= MIN_POP_DENSITY) & (~pop_data_cache.mask)
                desert_mask = (data_masked.data < 0) & (~data_masked.mask) & pop_mask
                
                ax.imshow(np.ones_like(desert_mask), cmap=colors.ListedColormap(['#00000000']), origin='upper')
                
                # Plot hatching for the desert areas
                ax.contourf(
                    desert_mask,
                    levels=[0.5, 1],
                    hatches=['////\\\\\\\\'],  # Crosshatch: forward + backward slashes
                    colors='none',
    ...
    # --- Stops ---
    stops_group = folium.FeatureGroup(name="Service Quality Stops", show=True).add_to(m_raster)
    stops_to_plot = stop_quality_gdf_cropped.copy()
    ...
    # Ensure necessary columns exist before applying cleaning
    if 'route_type' in stops_to_plot.columns:
        stops_to_plot['route_type'] = stops_to_plot['route_type'].apply(clean_stop_value)
    if 'route_names' in stops_to_plot.columns:
    ...
    for _, row in stops_to_plot.iterrows():
        ...
        <b>Quality:</b> {row.get('service_quality','')}<br>
        <b>Frequency:</b> {row.get('frequency','')}<br>
        <b>Routes:</b> {row.get('route_names', '')}<br>
        <b>Type:</b> {row.get('route_type', '')}
        """
        folium.CircleMarker(
            location=[row.geometry.y, row.geometry.x],
...
def get_raster_stats(level_of_service_path,aoi_geom):
    # --- File paths ---
    ...
    # We must project the AOI to the raster's CRS before reading
    try:
        with rasterio.open(pop_filepath) as src:
            raster_crs = src.crs
        aoi_projected = aoi_geom.to_crs(raster_crs)
    except Exception:
        # Fallback if raster file is not readable yet (shouldn't happen here, but safer)
        aoi_projected = aoi_geom


    # --- Read rasters ---
    pop_data, _, _ = raster_utils.read_raster(pop_filepath, aoi=aoi_projected, projected=False)
    diff_data, _, _ = raster_utils.read_raster(diff_filepath, aoi=aoi_projected, projected=False)
    offer_data, _, _ = raster_utils.read_raster(offer_filepath, aoi=aoi_projected, projected=False)
    ...
    # Insufficient coverage: difference < 0 AND population > 0
    ...
    # Map NaN (No coverage) to index 12 temporarily
    ...
    # Handle the population with NaN offer values (index 12)
    pop_by_offer["No Service"] = np.nansum(pop_data[(offer_int == 12) & valid_mask])
    ...
    return pop_offer_df, pct_insufficient
...
if st.session_state.level_of_service_gdf is not None:
    ...
    if show_step3_results:
        ...
        # --- Styling Function ---
        def color_density_matrix_display(df):
            def color_by_service(service):
                return f"background-color: {GENERAL_COLORS.get(service, 'white')}; color: white; text-align: center;"
        ...
        with col_density:
            ...
            styled_density_matrix = color_density_matrix_display(density_matrix_display_data)
        ...
        if run_raster_clicked and not st.session_state.raster_ready:
            ...
            if population_file is None:
                ...
                with st.spinner("Calculating accessibility rasters (Offer, Demand, Difference)..."):
                    ...
                    # Clear summary stats so they are re-calculated from new rasters
                    st.session_state.pop_offer_df = None
                    st.session_state.pct_insufficient = None
                    # Clear raster map so it is re-generated
                    st.session_state.raster_map = None
        ...
        if st.session_state.raster_ready:
            ...
            if st.session_state.raster_map is None:
                aoi_geom = st.session_state.aoi
                stop_quality_gdf_cropped = st.session_state.stop_quality_gdf_cropped.copy()
                raster_map = get_raster_map(aoi_geom,addr,stop_quality_gdf_cropped)
                
                # Save as HTML string
                st.session_state.raster_map = save_map_to_html(raster_map)
            
            # Display saved HTML map
            display_saved_map(st.session_state.raster_map, width=1200, height=600)

            # --- Population per Service Category Table ---
            st.markdown("---")
            st.subheader("📊 Accessibility Summary")

            # Load or compute population table
            if st.session_state.pop_offer_df is None: 
                aoi_geom = st.session_state.aoi
                pop_offer_df, pct_insufficient = get_raster_stats(level_of_service_path, aoi_geom)
                st.session_state.pop_offer_df = pop_offer_df
                st.session_state.pct_insufficient = pct_insufficient
            else:
                pop_offer_df = st.session_state.pop_offer_df
                pct_insufficient = st.session_state.pct_insufficient
            ...
            # Optional: Uncomment to display styled dataframe
            # st.dataframe(
            ...
            colors = []
            for level in pop_offer_df["Level of Service"]:
                grade = level[0] if level else None
                idx = int(level[1:]) - 1 if len(level) > 1 and level[1:].isdigit() else 0
                color_list = GRADE_COLOR_MAP.get(grade, ['#cccccc'])
                color = color_list[idx % len(color_list)]
                colors.append(color)

            # Function to show percentage only if ≥5%
            def autopct_func(pct):
                return f'{pct:.1f}%' if pct >= 5 else ''

            # Create pie chart (half frame)
            ...
            # Function to add group label only for A, B, C if total % >5%
            def add_group_label(group_letter,color):
                if group_letter not in ['A', 'B', 'C']:
                    return  # skip D, E, F, None

                # Convert itertuples to plain tuples (no attribute names)
                rows = list(pop_offer_df.itertuples(index=False, name=None))  # returns tuples
                # Column positions:
                # 0 -> Level of Service
                # 1 -> Population
                # 2 -> % of Total
                group_slices = [row for row in rows if row[0].startswith(group_letter)]
                total_pct = sum(row[2] for row in group_slices)  # % of Total column

                if total_pct > 5:
                    wedge_indices = [i for i, lvl in enumerate(pop_offer_df["Level of Service"]) if lvl.startswith(group_letter)]
                    theta1 = wedges[wedge_indices[0]].theta1
                    theta2 = wedges[wedge_indices[-1]].theta2
                    mid_angle = (theta1 + theta2) / 2

                    r = 0.3  # radius for label placement
                    x = r * np.cos(np.deg2rad(mid_angle))
                    y = r * np.sin(np.deg2rad(mid_angle))

                    ax.text(x, y, group_letter, ha='center', va='center', fontsize=16, fontweight='bold', color=color)
                    
            # Add group labels A, B, C
            for grp in ['A', 'B', 'C']:
                color = GENERAL_COLORS[grp]
                add_group_label(grp,color)

            # Hide wedge labels (only show autopct)
            for t in texts:
                t.set_visible(False)
